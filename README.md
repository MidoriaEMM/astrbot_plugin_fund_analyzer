...
- **默认**：纯文本结论摘要（投票、方向、简明操作建议等），无长段阶段日志
- **图片报告**（可选）：在指令尾部加 `发图` / `出图` / `要图` / `图片`，将 HTML 报告渲染为图（与结论摘要一并发送；渲染失败时仅文本）
- **详进度**（可选）：加 `详进度` / `详细进度` / `显示进度` 时，额外输出各 LLM 阶段说明
...
> 💡 **使用方法**：`股票智能分析 161226` — 共约 9 次 AI 对话，约 3～5 分钟；**默认仅文本结论**。需要报告图时加 **`发图`**（如 `股票智能分析 600519 发图`）。
...
| 命令                  | 说明                            | 示例                  |
| --------------------- | ------------------------------- | --------------------- |
| `股票智能分析 [代码] [发图]` | ⚖️ 多智能体博弈（默认纯文本结论；尾部 **`发图`** 等可选出报告图） | `股票智能分析 161226` |
...
| `量化精选股票 [候选只数] [输出条数]` | \|涨跌幅\| 前筛 + 排序（默认 150/10）；**默认剔除北交所、创业板、科创板**；**默认纯文本榜单**；需报告图时加 **发图**/**出图** 等；可用 **含北交**/**含创业板**/**含科技** 等；**去涨停** 等；同条中 **去*** 优先于 **含*** | `量化精选股票 150 10`、`量化精选股票 发图 150 10` |
| `基金 [代码]`              | 查询基金实时行情                            | `基金 161226`            |
| `股票 <代码>`              | 查询 A 股实时行情  | `股票 000001`            |
| `基金分析 [代码]`          | 技术指标分析       | `基金分析 161226`        |
| `量化分析 [代码]`          | 绩效/技术/回测（**无 LLM**）；**默认文本**；加 **发图** 等可选报告图 | `量化分析 161226`、`量化分析 161226 发图`        |
