"""将 ``fetch_quotes`` 产生的类 Tickflow 行情 dict 映射为与本项目 StockInfo 对齐字段。"""
...
def _ratio_to_pct(ratio: Any) -> float:
    """ext 中年涨跌幅、振幅、换手率等为小数比例时转为百分比数值。"""
...
def quote_to_stock_fields(quote: dict[str, Any], raw_code_display: str) -> dict[str, Any]:
    """
    把 ``fetch_quotes`` / ``fetch_quote_realtime`` 返回的单条 dict 转为
    ``StockInfo(**fields)`` 可用的字段（与 Tickflow 版字段名一致）。
    """
